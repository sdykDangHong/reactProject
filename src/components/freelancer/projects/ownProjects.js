...
class OwnProject extends Component{
    ...
    getOwnProjectList(){
        Http({
            url:`${Url.Bussiness}/agreement/selectFreelancerProjects.shtml`,
            method:"get",
            params:{
                freelancerId:this.props.customerId,
                pageNum:this.state.search.page,
                pageSize:10,
                sort:this.state.search.sort,
                projName:this.state.search.title
            }
        }).then((res)=>{
            ...
            <div className="ownProjectList">
                <div className="projectSearch">
                    ...
                    {this.state.ownProject.list.map((item)=>
                        <div className="projectItem" key={item.demandId}>
                            <div className="line firstLine clear">
                                <div className="lineItem left">{this.returnFirstSkill(item.tagName)}</div>
                                <div className="lineItem center">{item.title}</div>
                                <div className="lineItem right">发布日期：{formatTime(item.createTime,"Y-M-D")}</div>
                            </div>
                            <div className="line secondLine clear">
                                <div className="lineItem left">项目预算：{item.projectTotalAmount}元</div>
                            </div>
